    ...
    <div className='shadow-gray-700 shadow-sm w-full mx-auto z-[100] fixed top-0 h-[60px] bg-gray-900 text-white' onClick={(e)=>setShow(false)}>
      <div className='max-w-[1024px] relative w-10/12 h-full mx-auto text-xl flex justify-around items-center'>
        ...
        <div className={`${show ? 'flex' :'hidden'} absolute top-0 right-[-6%] bg-neutral-800 border-2 border-neutral-700 h-[240px] w-[190px] rounded-md mt-[20px] opacity-90 flex-col`}>
                ...
                <Link to='/login' onClick={()=>setShow(false)} className='border border-gray-700 transition-all duration-200 p-1  hover:scale-95 rounded-md hover:text-gray-200 px-2 bg-gradient-to-r from-indigo-500 to-pink-500'>Login</Link>
                <Link to='/signup'  onClick={()=>setShow(false)} className='border border-gray-700 transition-all duration-200 p-1  hover:scale-95 rounded-md hover:text-gray-200 px-2 bg-gradient-to-r from-indigo-500 to-pink-500'>SignUp</Link>
                </div>
        </div>
